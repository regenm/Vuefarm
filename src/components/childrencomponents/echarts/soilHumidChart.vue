...
// 模拟晴天土壤湿度数据（白天降低，夜间回升）
function generateSoilMoistureData(count) {
  const data = []
  let base = 60 + Math.random() * 10 // 起始湿度（60%）
  for (let i = 0; i < count; i++) {
    const hour = (i * 24) / count // 计算当前点对应的小时
    if (hour >= 6 && hour <= 18) {
      // 白天（6点到18点），湿度逐渐降低
      base -= Math.random() * 0.5
      if (base < 30) base = 30 // 保持最低湿度为30%
    } else {
      // 晚上（18点到6点），湿度回升
      base += Math.random() * 1
      if (base > 70) base = 70 // 保持最高湿度为70%
    }
...
function updateChart() {
  ...
  const now = new Date()
  const xData = []
  const yData = generateSoilMoistureData(count)
